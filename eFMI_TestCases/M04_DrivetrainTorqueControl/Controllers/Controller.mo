...
model Controller
  ...
  Modelica.Blocks.Math.Gain gear(k=gearRatio)
  ...
  Utilities.LimPI_withFeedForward PI(
    ...
    k=k_PI,
    Ti=Ti_PI,
    Ni=Ni_PI) annotation (Placement(transformation(extent={{40,-10},{60,10}})));
  Modelica.Blocks.Math.Gain gear1(k=1/gearRatio)
...
equation
  ...
  connect(PI.y, gear1.u)
    annotation (Line(
      points={{61,0},{72.8,0}},
      color = {0,0,127}));
  connect(wRel, PI.u_m) annotation (Line(points={{0,-120},{0,-70},{50,-70},{50,
          -12}},
        color={0,0,127}));
  connect(gear1.y, M_motor)
    annotation (Line(points={{86.6,0},{110.25,0}}, color={0,0,127}));
  ...
  connect(inverseBlockConstraints.y1, gear.u) annotation (Line(points={{-8.75,0},
          {-2,0},{-2,6},{6.8,6}}, color={0,0,127}));
  connect(approxPlant.w_rel, PI.u_s) annotation (Line(points={{-30,-11},{-30,
          -24},{32,-24},{32,0},{38,0}}, color={0,0,127}));
end Controller;
