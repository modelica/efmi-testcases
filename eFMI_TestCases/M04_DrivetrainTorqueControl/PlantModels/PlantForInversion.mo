...
model PlantForInversion
  ...
  Modelica.Blocks.Interfaces.RealOutput M_l "Input signal 2 (u1 = u2)"
    annotation (Placement(transformation(extent={{100,-20},{140,20}}),
        iconTransformation(extent={{100,-20},{140,20}})));
  Modelica.Blocks.Interfaces.RealInput  M_m "Output signal 1 (y1 = y2)"
  ...
  Modelica.Blocks.Math.Gain gainAcc(k=k_accCor) annotation (Placement(
        transformation(
        extent={{10,-10},{-10,10}},
        rotation=0,
        origin={20,50})));
  ...
  Driveline
    ...
    gearRatio=gearRatio,
    c=c_res,
    d=d_res) annotation (Placement(transformation(extent={{-40,-20},{0,20}})));
equation
  connect(loadTorque.flange_b, fixed.flange) annotation (Line(
      points={{70,0},{76,0}},
      color={0,0,0},
      smooth=Smooth.None));
  connect(torqueDist.tau, gainAcc.y)
    annotation (Line(points={{2,50},{9,50}},           color={0,0,127}));
  connect(gainAcc.u,loadTorque. tau) annotation (Line(
      points={{32,50},{52,50},{52,11}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(M_l, gainAcc.u) annotation (Line(
  ...
  connect(torqueDrive.flange, driveline.flange_a)
    annotation (Line(points={{-60,0},{-40,0}}, color={0,0,0}));
  connect(driveline.flange_b, loadTorque.flange_a)
    annotation (Line(points={{0,0},{50,0}}, color={0,0,0}));
  connect(driveline.w_rel, w_rel) annotation (Line(points={{-20,-22},{-20,-94},
          {0,-94},{0,-110}}, color={0,0,127}));
  connect(torqueDist.flange, driveline.flange_b) annotation (Line(points={{-20,
          50},{-28,50},{-28,32},{20,32},{20,0},{0,0}}, color={0,0,0}));
  annotation (
