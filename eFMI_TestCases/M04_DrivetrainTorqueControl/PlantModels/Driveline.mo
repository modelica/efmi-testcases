...
model Driveline "Physical parts of the driveline between motor and wheel axis"
  Modelica.Mechanics.Rotational.Components.Inertia inertiaDrivelineIn(
    phi(start=0),
    w(start=0),
    J=J_M) annotation (Placement(transformation(extent={{-50,-10},{-30,10}})));
  Modelica.Mechanics.Rotational.Components.SpringDamper Gear(
    phi_rel0(displayUnit="rad"),
    phi_rel(fixed=true),
    c=c,
    d=d) annotation (Placement(transformation(extent={{10,-10},{30,10}})));
  ...
  parameter Modelica.SIunits.Inertia J_M "Moment of inertia";
  parameter Real gearRatio "Transmission ratio (flange_a.phi/flange_b.phi)";
  parameter Modelica.SIunits.RotationalSpringConstant c "Spring constant";
  parameter Modelica.SIunits.RotationalDampingConstant d "Damping constant";
...
equation
  connect(inertiaDrivelineIn.flange_b,idealGear. flange_a) annotation (Line(
      points={{-30,0},{-20,0}},
      color={0,0,0},
      smooth=Smooth.None));
  connect(idealGear.flange_b,Gear. flange_a) annotation (Line(
      points={{0,0},{10,0}},
      color={0,0,0},
      smooth=Smooth.None));
  connect(relSpeedSensor.flange_a, idealGear.flange_b)
    annotation (Line(points={{10,-40},{0,-40},{0,0}}, color={0,0,0}));
  connect(relSpeedSensor.w_rel, w_rel) annotation (Line(points={{20,-51},{20,-60},
          {0,-60},{0,-110}}, color={0,0,127}));
  connect(Gear.flange_b, relSpeedSensor.flange_b)
    annotation (Line(points={{30,0},{40,0},{40,-40},{30,-40}}, color={0,0,0}));
