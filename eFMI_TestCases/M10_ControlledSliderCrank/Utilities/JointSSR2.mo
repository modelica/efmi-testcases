...
model JointSSR2
  ...
  parameter .Modelica.Units.SI.Length rod1Length(
    min = .Modelica.Constants.eps,
    start = 1)
    "Distance between the origins of the two spherical joints";
  parameter .Modelica.Units.SI.Mass rod1Mass(
    min = 0) = 0
  ...
  parameter .Modelica.Units.SI.Position rRod2_ib[3] = {1,0,0}
  ...
  parameter .Modelica.Units.SI.Diameter sphereDiameter = world.defaultJointLength
    "Diameter of the spheres representing the two spherical joints"
    annotation (Dialog(
      tab="Animation",
      group="if animation = true",
      enable=animation));
  parameter .Modelica.Units.SI.Diameter rod1Diameter = sphereDiameter/Types.Defaults.JointRodDiameterFraction
    "Diameter of rod 1 connecting the two spherical joints" annotation (
      Dialog(
      tab="Animation",
      group="if animation = true",
      enable=animation));
  parameter .Modelica.Units.SI.Diameter rod2Diameter = rod1Diameter
    "Diameter of rod 2 connecting the revolute joint and spherical joint 2"
    annotation (Dialog(
      tab="Animation",
      group="if animation = true",
      enable=animation));
  parameter .Modelica.Units.SI.Diameter revoluteDiameter = world.defaultJointWidth
    "Diameter of cylinder representing the revolute joint"
    annotation (Dialog(
      tab="Animation",
      group="if animation = true",
      enable=animation));
  parameter .Modelica.Units.SI.Distance revoluteLength = world.defaultJointLength
  ...
  .Modelica.Mechanics.MultiBody.Joints.SphericalSpherical rod1(
    ...
    annotation (Placement(transformation(extent={{-89,-20},{-49,20}})));
  .Modelica.Mechanics.MultiBody.Parts.FixedTranslation rod2(
    ...
    specularCoefficient = specularCoefficient,
    r = rRod2_ib)
    annotation (Placement(transformation(extent={{15,-20},{-25,20}})));
  ...
  */
  ...
  aux = cross(revolute.e, rRod2_ib)
    * .Modelica.Mechanics.MultiBody.Frames.resolveRelative(
      ...
      rod1.frame_b.R);
  f_rod = (
    -revolute.tau - revolute.e*(
      frame_ib.t + frame_im.t + cross(rRod2_ib, frame_im.f)
      - cross(
        rRod2_ib,
        .Modelica.Mechanics.MultiBody.Frames.resolveRelative(
          rod1.f_b_a1,
  ...
  if checkTotalPower then
    totalPower =
      ...
      * .Modelica.Mechanics.MultiBody.Frames.resolve2(
        ...
        der(frame_im.r_0))
      + frame_a.t
      * .Modelica.Mechanics.MultiBody.Frames.angularVelocity2(frame_a.R)
      + frame_b.t
      * .Modelica.Mechanics.MultiBody.Frames.angularVelocity2(frame_b.R)
      + frame_ib.t
      * .Modelica.Mechanics.MultiBody.Frames.angularVelocity2(frame_ib.R)
      + frame_im.t
      * .Modelica.Mechanics.MultiBody.Frames.angularVelocity2(frame_im.R)
      + axis.tau
      * der(axis.phi)
      + bearing.tau
      * der(bearing.phi)
      + (-rod1Mass)
      * (der(rod1.v_CM_0) - world.gravityAcceleration(rod1.r_CM_0))
      * rod1.v_CM_0;
  else
    totalPower = 0;
  end if;
  ...
  connect(revolute.axis, axis)
    annotation (Line(
      points={{55,20},{55,60},{90,60},{90,80},{100,80}}));
