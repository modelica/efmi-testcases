...
model JointSSR2
  ...
  parameter Modelica.SIunits.Length rod1Length(min=Modelica.Constants.eps,
      start=1) "Distance between the origins of the two spherical joints";
  parameter Modelica.SIunits.Mass rod1Mass(min=0)=0
  ...
  parameter Modelica.SIunits.Position rRod2_ib[3]={1,0,0}
  ...
  parameter Modelica.SIunits.Diameter sphereDiameter=world.defaultJointLength
    "Diameter of the spheres representing the two spherical joints" annotation (
     Dialog(
      tab="Animation",
  ...
  parameter Modelica.SIunits.Diameter rod1Diameter=sphereDiameter/Types.Defaults.JointRodDiameterFraction
    "Diameter of rod 1 connecting the two spherical joints" annotation (Dialog(
      tab="Animation",
      group="if animation = true",
      enable=animation));
  input Types.Color rod1Color=Modelica.Mechanics.MultiBody.Types.Defaults.
    ...
    annotation (Dialog(colorSelector=true, tab="Animation", group="if animation = true", enable=animation));
  parameter Modelica.SIunits.Diameter rod2Diameter=rod1Diameter
  ...
  input Types.Color rod2Color=rod1Color
    ...
    annotation (Dialog(colorSelector=true, tab="Animation", group="if animation = true", enable=animation));
  parameter Modelica.SIunits.Diameter revoluteDiameter=world.defaultJointWidth
    "Diameter of cylinder representing the revolute joint" annotation (Dialog(
      tab="Animation",
      group="if animation = true",
      enable=animation));
  parameter Modelica.SIunits.Distance revoluteLength=world.defaultJointLength
    "Length of cylinder representing the revolute joint" annotation (Dialog(
      tab="Animation",
      group="if animation = true",
      enable=animation));
...
eFMI_TestCases.M10_ControlledSliderCrank.Utilities.RevoluteWithLengthConstraint2
    revolute(
    animation=animation,
    ...
    specularCoefficient=specularCoefficient, positiveBranch=positiveBranch)
...
equation
  ...
  */
  ...
  aux = cross(revolute.e, rRod2_ib)*
    Modelica.Mechanics.MultiBody.Frames.resolveRelative(
    rod1.eRod_a,
    rod1.frame_a.R,
    rod1.frame_b.R);
  f_rod = (-revolute.tau - revolute.e*(frame_ib.t + frame_im.t + cross(rRod2_ib,
    frame_im.f) - cross(rRod2_ib,
    Modelica.Mechanics.MultiBody.Frames.resolveRelative(
    rod1.f_b_a1,
    rod1.frame_a.R,
    rod1.frame_b.R))))/noEvent(if abs(aux) < 1e-10 then 1e-10 else aux);

  // Measure power for test purposes
  if checkTotalPower then
    totalPower = frame_a.f*Modelica.Mechanics.MultiBody.Frames.resolve2(frame_a.R,
      der(frame_a.r_0)) + frame_b.f*
      Modelica.Mechanics.MultiBody.Frames.resolve2(frame_b.R, der(frame_b.r_0)) +
      frame_ib.f*Modelica.Mechanics.MultiBody.Frames.resolve2(frame_ib.R, der(
      frame_ib.r_0)) + frame_im.f*Modelica.Mechanics.MultiBody.Frames.resolve2(
      frame_im.R, der(frame_im.r_0)) + frame_a.t*
      Modelica.Mechanics.MultiBody.Frames.angularVelocity2(frame_a.R) + frame_b.t
      *Modelica.Mechanics.MultiBody.Frames.angularVelocity2(frame_b.R) +
      frame_ib.t*Modelica.Mechanics.MultiBody.Frames.angularVelocity2(frame_ib.R)
       + frame_im.t*Modelica.Mechanics.MultiBody.Frames.angularVelocity2(
      frame_im.R) + axis.tau*der(axis.phi) + bearing.tau*der(bearing.phi) + (-
      rod1Mass)*(der(rod1.v_CM_0) - world.gravityAcceleration(rod1.r_CM_0))*
      rod1.v_CM_0;
  else
    totalPower = 0;
  end if;
