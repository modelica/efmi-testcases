...
model Controller2
  ...
  Modelica.Blocks.Math.Feedback feedback
    annotation (Placement(transformation(
      extent = {{-10,-30},{10,-10}},
      rotation = 0)));
  PlantModels.SliderCrank2 sliderCrank2_1(
    world(
...
equation
  connect(feedback.y, gain.u)
    annotation (Line(
      points = {{9,-20},{18,-20}},
      color = {0,0,127}));
  connect(sliderCrank2_1.s, inverseBlockConstraints.u2)
    annotation (Line(
      points = {{-3.4,20},{-20.4,20}},
      color = {0,0,127}));
  ...
  connect(inverseBlockConstraints.y1, add.u1)
    annotation (Line(
      ...
      color = {0,0,127}));
  connect(sliderCrank2_1.phi, feedback.u1)
    annotation (Line(
      points={{-3.4,10.2},{-20,10.2},{-20,-20},{-8,-20}},
      color = {0,0,127}));
  ...
  connect(add.y, tau)
    annotation (Line(
      points = {{80.6,0},{110,0}},
      color = {0,0,127}));
  connect(sliderCrank2_1.tau, inverseBlockConstraints.y2)
    annotation (Line(
