...
block DriveTrain
  ...
  .Modelica.Blocks.Interfaces.RealOutput w_diff(
    min = -100,
    max = 100)
...
equation
  ...
  connect(diff.y, w_diff)
    annotation (Line(points={{89,60},{110,60}}, color={0,0,127}));
  connect(speedSensor.w, diff.u2)
    annotation (Line(points={{75,0},{80,0},{80,52}}, color={0,0,127}));
  connect(w_ref, diff.u1)
    annotation (Line(points={{-120,0},{-90,0},{-90,60},{72,60}},                                                                     color={0,0,127}));
  connect(criticalDamping.y, speed.w_ref)
    annotation (Line(points={{-63.2,0},{-56,0}},                                                                     color={0,0,127}));
  connect(w_ref, criticalDamping.u)
    annotation (Line(points={{-120,0},{-81.6,0}}, color={0,0,127}));
end DriveTrain;
