...
model PIDController
  ...
  .Modelica.Blocks.Interfaces.RealInput wLoadRef(
    min = -1e5,
    max = 1e5)
    "Reference angular velocity of load"
    annotation (Placement(transformation(extent = {{-140,-20},{-100,20}})));
  .Modelica.Blocks.Interfaces.RealInput wMotor(
    min = -1e5,
    max = 1e5)
    ...
    annotation (Placement(transformation(
      ...
      rotation = 90,
      origin = {0,-120})));
  .Modelica.Blocks.Interfaces.RealOutput vMotor(
    min = -1e7,
    max = 1e7)
    "Motor voltage"
  ...
  Utilities.Limiter limiter(uMax(
    min = 10,
    max = 1e6) = 400)
