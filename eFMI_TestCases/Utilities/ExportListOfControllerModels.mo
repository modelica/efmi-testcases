...
function ExportListOfControllerModels "Exports a sorted list of all test cases with their controller models into a text file (everything in subpackes \"Tests\" with correct annotation __iti_namesAtInterface)"
  ...
  input String outputDir = "eFMUs" "Name of directory where the list is saved";

  output Boolean ok;

algorithm

  ok := Internal.runFunctionWithModelInformation("\teFMI_TestCases.Utilities.Internal.exportListOfControllerModels(\""+fileName+"\", \""+outputDir+"\", info);\n");
