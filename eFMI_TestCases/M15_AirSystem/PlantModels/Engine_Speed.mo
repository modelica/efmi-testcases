...
model Engine_Speed "Crank shaft speed dynamics"
  ...
  Modelica.Blocks.Interfaces.RealOutput n(start=Modelica.SIunits.Conversions.from_rpm(1000), fixed=true, unit="rad/s", displayUnit="rpm") "Crank shaft speed"
    annotation (Placement(transformation(extent={{100,-10},{120,10}})));

  parameter Modelica.SIunits.Time dt_delay = 0.05 "Injection torque delay time";
  parameter Modelica.SIunits.SpecificInternalEnergy Hu = 46.4e6 "Fuel lower heating value";
  parameter Modelica.SIunits.Efficiency eta_i = 0.8276 "Volumetric efficiency";
  parameter Modelica.SIunits.Inertia I = 100 "Crank shaft load inertia";

  // added 12.12.2019 by O. Lenord to allow reasonable loss power calculation
  parameter Modelica.SIunits.Area A_p = 10e-4 "piston friction area";
  parameter Modelica.SIunits.CoefficientOfFriction mue_p = 0.98 "piston coeff. of friction";
  parameter Modelica.SIunits.Length s_p = 0.98 "piston travel";
  parameter Modelica.SIunits.Volume V_p = 1e-3 "pump volume";
  parameter Modelica.SIunits.RotationalDampingConstant kb = 0.1 "brake friction";

  Modelica.SIunits.MassFlowRate m_dot_f_delayed "delayed fuel mass flow rate";

  parameter Boolean ignore_delay = false;

  Modelica.SIunits.Power Pf(displayUnit="kW") "Friction power";
  Modelica.SIunits.Power Pp(displayUnit="kW") "Pumping power";
  Modelica.SIunits.Power Pb(displayUnit="kW") "Load power";
  Modelica.SIunits.Power Pc(displayUnit="kW") "Combustion power";
