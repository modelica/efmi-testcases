...
block SimpleDrivenVehicleWithSlip
  extends Modelica.Blocks.Icons.Block;
   ...
   Modelica.Blocks.Interfaces.RealInput v_ref(min=-1.0e7, max=1.0e7)
  ...
  Utilities.WheelWithLinearSlip wheel(
    ...
    J=1,
    stiffness=20000,
    v_min=0.1) annotation (Placement(transformation(extent={{0,-10},{20,10}})));
  Modelica.Mechanics.Rotational.Sources.Speed speed(exact=true,
                                                    f_crit=15)
    annotation (Placement(transformation(extent={{-30,-10},{-10,10}})));
  Modelica.Blocks.Math.Gain gain(final k=1/wheel.wheelRadius)
...
equation
  ...
  connect(mass.flange_b, speedSensor.flange)
    annotation (Line(points={{50,0},{60,0}}, color={0,127,0}));
  connect(speedSensor.v, w)
    annotation (Line(points={{81,0},{110,0}}, color={0,0,127}));
  connect(gain.y, criticalDamping.u) annotation (Line(points={{-71,0},{-62,0}},   color={0,0,127}));
  connect(criticalDamping.y, speed.w_ref) annotation (Line(points={{-39,0},{-32,
          0}},                                                                         color={0,0,127}));
