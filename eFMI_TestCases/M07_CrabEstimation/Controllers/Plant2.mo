...
model Plant2
  ...
  Real x[nx](
    start = {angle_init,w_init,s_init,v_init},
    each fixed = true)
    "States";
  Real u[nu]
    "Inputs";
...
equation
  ...
  dx4aux =
    (u[1] + m*m*g*l*l*cos(x[1])*sin(x[1])/(J + m*l*l) + m*l*sin(x[1])*x[2]*x[2])
    / (mCrab + m - m*m*l*l*cos(x[1])*cos(x[1])/(J + m*l*l));
  der(x) = {
    x[2],
    (-m*g*l*sin(x[1]) - m*l*cos(x[1])*dx4aux)/(J + m*l*l),
    x[4],
    dx4aux};
