...
model SimpleDrivetrain "Very simple model of a driveline"
  ...
  Modelica.Mechanics.Rotational.Components.Inertia inertia1(
    ...
    J=1,
    a(fixed=true, start=0)) annotation (Placement(transformation(extent={{-50,-10},
            {-30,10}})));
...
equation
  connect(spring.flange_b,inertia2. flange_a)
    annotation (Line(points={{10,0},{28,0}}));
  connect(inertia1.flange_b,spring. flange_a)
    annotation (Line(points={{-30,0},{-10,0}}));
  connect(torque.flange,inertia1. flange_a)
    annotation (Line(points={{-60,0},{-50,0}}));
  connect(speedSensor.flange,inertia1. flange_b)
    annotation (Line(points={{-10,-40},{-20,-40},{-20,0},{-30,0}}));
  connect(loadTorque.flange,inertia2. flange_b)
    annotation (Line(points={{60,0},{48,0}}));
  connect(tau, torque.tau) annotation (Line(points={{-120,0},{-82,0}},
                   color={0,0,127}));
  connect(speedSensor.w, w) annotation (Line(points={{11,-40},{96,-40},{96,0},{110,
          0}},                           color={0,0,127}));
